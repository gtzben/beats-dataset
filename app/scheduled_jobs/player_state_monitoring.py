...
@with_appcontext
def __check_activity(sp, sleep_time):
    """
    
    """

    # Format milliseconds to minutes and seconds for logger 
    ...
    while user_playback_state is not None:
        ...
        relevant_info = {
            ...
            "account":os.environ.get('SPOTIPY_ACCOUNT_ID'),
        ...
        log_msg = (
                    f"Spotify Account: {os.environ.get('SPOTIPY_ACCOUNT_ID')} | "
                    f"Song: {relevant_info['track_name']} | "
                    f"Context: {STUDY_PLAYLISTS.get(relevant_info['context_uri'], 'Other')} | "
                    f"Device: {relevant_info['device_type']} | "
                    f"Progress Track: {ms_to_min_sec(relevant_info['progress_track_ms'])} | "
                    f"Last Playback Change: {datetime.fromtimestamp(user_playback_state['timestamp']/1000)} | " 
                    f"Currently Playing: {relevant_info['is_playing']}"
                )
...
@with_appcontext
def __send_survey(account_email):
    """
    ...
    with mail.connect() as conn:
        ...
        try:
            conn.send(msg)
            LOGGER.info(f"Survey sent to participant {account_email}")
        except Exception:
            LOGGER.exception(f"Unable to send email to {account_email}")
            sys.exit (-1)
...
def monitor_playback_state(spotify_email,sleep_time):
    ...
    #
    activity_detected = __check_activity(spotify_client, sleep_time)

    #
    if activity_detected:
        __send_survey(spotify_email)
