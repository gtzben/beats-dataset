...
def process_signal(data, signal_name, columns):
    """
    ...
    if "unix_timestamp" in columns:
        ...
        timestamp = [
            round(signal["timestampStart"] + i * (1e6 / signal["samplingFrequency"]))
...
def process_avro_file_from_s3(file_key, s3_client):
    ...
    df_signals = [process_signal(data, "accelerometer", ["unix_timestamp", "x", "y", "z"]),
    process_signal(data, "gyroscope", ["unix_timestamp", "x", "y", "z"]),
    process_signal(data, "eda", ["unix_timestamp", "eda"]),
    process_signal(data, "temperature", ["unix_timestamp", "temperature"]),
    process_signal(data, "bvp", ["unix_timestamp", "bvp"]),
    process_signal(data, "steps", ["unix_timestamp", "steps"]),
    process_signal(data, "tags", ["tagsTimeMicros"]),
    process_signal(data, "systolicPeaks", ["peaksTimeNanos"])]
