...
from app.routes.survey.forms import ParticipantLoginForm, BEATS_Psychometrics
...
def pre_study():
    ...
    table_headers = {f"{request.blueprint}_demo_data": [],
                    ...
                    f"{request.blueprint}_tipi_data": ["Statement", "Disagree strongly", "Disagree moderately", "Disagree a little",
                                  "Neither agree nor disagree", "Agree a little", "Agree moderately",
                                    "Agree strongly"],

                    f"{request.blueprint}_panas_data": ["Statement", "Very slighlty or not at all", "A little", "Moderately", "Quite a bit", "Extremely"],

                    f"{request.blueprint}_pss_data": ["Statement", "Never", "Almost Never", "Sometimes", "Fairly Often", "Very Often"],

                    f"{request.blueprint}_phq9_data": ["Statement", "Not at all", "Several days", "More than half the days", "Nearly every day"],

                    f"{request.blueprint}_stompr_data":["Statement", "Dislikes strongly", "Dislike moderately", "Dislike a little", "Neither like or dislike",
                                   "Like a little", "Like moderately", "Like strongly"],
                    
                    f"{request.blueprint}_gms_data":["Statement", "Completly disagree", "Strongly disagree", "Disagree", "Neither agree nor disagree",
                                "Agree", "Strongly agree", "Completely agree"]}
    
    survey_instructions = {f"{request.blueprint}_demo_data": ("In this section, we will ask about your gender, age, and other demographic details " +
                    ...
                    f"{request.blueprint}_tipi_data": ("Here are a number of personality traits that may or may not apply to you.",
                    ...
                    f"{request.blueprint}_panas_data": ("Review the list of feelings and emotions.",
                    ...
                    f"{request.blueprint}_pss_data": ("The questions below ask about your feelings and thoughts over the past month.",
                                   "Select how often you felt this way (1 = never, 5 = very often)."),

                    f"{request.blueprint}_phq9_data": ("Over the last two weeks, how often have you been bothered by the following problems?",
                                   "Choose the option that best represents your experience (1 = not at all, 4 = nearly every day)."),

                    f"{request.blueprint}_stompr_data":("Read each music genre or statement about preferences",
                                   "Select how much you enjoy or agree with each (1 = strongly dislike, 7 = strongly like)."),
                    
                    f"{request.blueprint}_gms_data":("Answer each question about your musical habits, skills, and experiences.",
    ...
    # print(session.get(f'{request.blueprint}_demo_data'))
...
def post_study():
    ...
    data = {"pid": session.get(f'{request.blueprint}_participant_pid')}

    #
    response = requests.post(url=url_for("api.participantconcluderesource", _external=True), json=data)
    api_data = response.json()
    ...
    if response.status_code == HTTPStatus.OK:
        ...
        if session.get(f'{request.blueprint}_is_withdrawn'):
            ...
            return render_template('post_study.html', title='Post-study', blueprint=request.blueprint, logged_in=bool(session.get(f'{request.blueprint}_participant_pid')))
